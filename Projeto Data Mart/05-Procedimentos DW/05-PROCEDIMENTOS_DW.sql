...
AS
BEGIN
    DECLARE @ID_TEMPO INT, @QTD SMALLINT, @VALOR NUMERIC(10,2);
    DECLARE CURSOR_F CURSOR FOR SELECT F.ID_TEMPO, SUM(F.QTD), SUM(F.VALOR)
                                FROM AMBIENTE_DIMENSIONAL.FATO_ACOMPANHAMENTO F
                                         JOIN DIM_SERVICO DS ON F.ID_SERVICO = DS.ID
                                         JOIN DIM_TEMPO DT ON DT.ID = F.ID_TEMPO
    ...
    WHILE(@@FETCH_STATUS = 0)
    BEGIN
        INSERT INTO FATO_ACOMPANHAMENTO_CONCLUIDO(ID_TEMPO, QTD, VALOR)
        VALUES(@ID_TEMPO, @QTD, @VALOR);
        FETCH CURSOR_F INTO @ID_TEMPO, @QTD, @VALOR;
    END
    CLOSE CURSOR_F;
    DEALLOCATE CURSOR_F;
END
