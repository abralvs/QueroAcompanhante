...
EXEC AMBIENTE_DIMENSIONAL.SP_POVOA_FATO_ACOMPANHAMENTO_CONCLUIDO_MES         ;

SELECT * FROM AMBIENTE_DIMENSIONAL.FATO_ACOMPANHAMENTO
SELECT * FROM AMBIENTE_DIMENSIONAL.FATO_ACOMPANHAMENTO_CONCLUIDO_MES
...
CREATE PROCEDURE AMBIENTE_DIMENSIONAL.SP_POVOA_FATO_ACOMPANHAMENTO_CONCLUIDO_MES
AS
BEGIN
    DECLARE @ID_TEMPO INT, @QTD SMALLINT, @VALOR NUMERIC(10,2),@MES INT,@ANO INT;
    
	DECLARE CURSOR_F CURSOR FOR SELECT DT.MES,DT.ANO, SUM(F.QTD), SUM(F.VALOR)
                                FROM AMBIENTE_DIMENSIONAL.FATO_ACOMPANHAMENTO F
                                         JOIN AMBIENTE_DIMENSIONAL.DIM_SERVICO DS ON F.ID_SERVICO = DS.ID
                                         JOIN AMBIENTE_DIMENSIONAL.DIM_TEMPO DT ON DT.ID = F.ID_TEMPO 
                                WHERE DS.status = 'CONCLUIDA'
                                GROUP BY DT.MES, DT.ANO;
	
    OPEN CURSOR_F;
    FETCH CURSOR_F INTO @MES, @ANO,@QTD, @VALOR;
    WHILE(@@FETCH_STATUS = 0)
    BEGIN
		
		SET @ID_TEMPO = (SELECT ID FROM AMBIENTE_DIMENSIONAL.DIM_TEMPO AS T 
						 WHERE (@MES = T.MES AND @ANO = T.ANO AND T.NIVEL = 'MES'))

        INSERT INTO FATO_ACOMPANHAMENTO_CONCLUIDO_MES(ID_TEMPO, QTD, VALOR)
        VALUES(@ID_TEMPO, @QTD, @VALOR);
