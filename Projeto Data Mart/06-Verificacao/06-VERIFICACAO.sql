...
BEGIN
	SELECT COUNT(FA.QTD) AS QUANTIDADE_ESPECIE, SUM(FA.VALOR) AS TOTAL FROM AMBIENTE_DIMENSIONAL.FATO_ACOMPANHAMENTO FA
	INNER JOIN AMBIENTE_DIMENSIONAL.DIM_TEMPO DT ON FA.ID_TEMPO = DT.ID
	INNER JOIN AMBIENTE_DIMENSIONAL.DIM_TRANSACAO AS TRANS ON (TRANS.id = FATO.id_transacao) AND (TRANS.tipo_pagamento = 'EM ESPECIE')
	WHERE (DT.DATA >= @INICIO_PERIODO AND DT.DATA <= @FIM_PERIODO);
...
ALTER PROCEDURE AMBIENTE_DIMENSIONAL.SP_13_ESTADOS_MAIS_COMUNS (@DATA_INICIO DATETIME,@DATA_LIMITE DATETIME)
AS
BEGIN
